...
def get_preds(model, dataset_iter, device, args, top_k=None, label_weights=None, rlls_infer=True):
    ...
    for (data, labels, _) in dataset_iter:
        ...
        if top_k is not None:
            top_k_preds.append(output.topk(
                top_k, dim=1).indices.cpu().data.numpy())
...
def evaluate(model, dataset_iter, device, args, label_weights=None):
    ...
    with torch.no_grad():
        preds, true_preds, top5_preds = get_preds(
            model, dataset_iter, device, args, top_k=5,
    ...
    total_top5_acc = np.sum(np.any(np.repeat(np.expand_dims(
        true_preds, 1), 5, axis=1) == top5_preds, axis=1)) / len(preds)
    ...
    accuracies["Macro F1"] = metrics.f1_score(
        true_preds, preds, average='macro')
    accuracies["Micro F1"] = metrics.f1_score(
        true_preds, preds, average='micro')
    accuracies["Weighted F1"] = metrics.f1_score(
        true_preds, preds, average='weighted')
    return accuracies
...
def train(model, dataset, epochs, args, lr=None, log_fn=None, milestones=None):
    ...
    if args.dataset == "mnist":
        ...
        warmup_scheduler = WarmUpLR(optimizer, math.ceil(
            dataset.labeled_len() / args.batch_size) * args.warm_epochs)

    for epoch in range(epochs):
        print("Epoch", epoch)
        for (data, target, weight) in dataset.iterate(batch_size=args.batch_size,
                                                      shuffle=True, split="labeled"):
