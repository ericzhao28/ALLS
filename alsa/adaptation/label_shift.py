...
def label_shift(network, dataset, args):
    ...
    source_dataset = dataset.iterate(
        args.infer_batch_size, shuffle=False, split="labeled" if args.reweight else "warmstart")
    with torch.no_grad():
        preds, true_preds = get_preds(
            network, source_dataset, args.device, args, label_weights=dataset.label_weights)
    source_priors = np.zeros(args.num_cls)
    for i in range(args.num_cls):
        source_priors[i] = float(
            len(np.where(preds == i)[0])) / len(preds)
    true_source_priors = np.zeros(args.num_cls)
    for i in range(args.num_cls):
        true_source_priors[i] = float(
            len(np.where(true_preds == i)[0])) / len(true_preds)
    ...
    target_dataset = dataset.iterate(
        args.infer_batch_size, shuffle=False, split="test")
    with torch.no_grad():
        preds, true_preds = get_preds(
            network, target_dataset, args.device, args, label_weights=dataset.label_weights)
    true_target_priors = np.zeros(args.num_cls)
    for i in range(args.num_cls):
        true_target_priors[i] = float(
            len(np.where(true_preds == i)[0])) / len(true_preds)
    target_priors = np.zeros(args.num_cls)
    for i in range(args.num_cls):
        target_priors[i] = float(
            len(np.where(preds == i)[0])) / len(preds)

    # Report true weights
    true_label_weights = true_target_priors / true_source_priors
    print("Optimal label weights:", np.array(
        100 * true_label_weights, dtype=np.int32))
    ...
    if args.shift_correction == "bbse":
        ...
        try:
            ...
            label_weights = np.linalg.inv(
                confuse +
                np.random.uniform(
                    0,
                    1e-6,
                    size=confuse.shape)) @ target_priors
    ...
    print("Diff weights l2 {}, mse {}, max {}".format(
        np.linalg.norm(diff_weights),
        np.mean(np.square(diff_weights)),
